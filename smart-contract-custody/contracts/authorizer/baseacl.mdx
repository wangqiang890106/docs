...
2. Implement the `contracts()` function. A list of smart contract addresses controlled by the `Authorizer` will be returned. The `Authorizer` will continue the review process only if the “to address" of the transaction is in the list. Otherwise, the `Authorizer` will directly reject the transaction.
